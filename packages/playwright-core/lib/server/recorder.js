...
class Recorder {
  ...
  async install(recorderApp, showRecorder) {
    if (showRecorder)
    ...
    await this._context.exposeBinding("__pw_recorderSetSelector", false, async ({ frame }, selector) => {
...
async function findFrameSelector(frame) {
  try {
    ...
    const selector = await injected.evaluate((injected2, element) => {
      return injected2.generateSelectorSimple(element, { testIdAttributeName: "", omitInternalEngines: true });
    }, frameElement);
    return selector;
  } catch (e) {
  }
